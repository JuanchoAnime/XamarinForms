...
namespace Xamarin.Agora.Full.Forms
{
    public  class AgoraServiceImplementation : IAgoraService
    {
        ...
        public string ImplementationPlatform => "Android";
        ...
        public void SetSpeakerEnabled(bool speaker)
        {
            ...
            if (!speaker)
            {
                AudioManager localAudioManager = (AudioManager)global::Android.App.Application.Context.GetSystemService(Context.AudioService);
        ...
        private VideoEncoderConfiguration GetVideoConfig(VideoAgoraProfile profile)
        {
            switch (profile)
            {
                case VideoAgoraProfile.Landscape720P:
                    return new VideoEncoderConfiguration(VideoEncoderConfiguration.VD1280x720, VideoEncoderConfiguration.FRAME_RATE.FrameRateFps15, 
                        VideoEncoderConfiguration.StandardBitrate, VideoEncoderConfiguration.ORIENTATION_MODE.OrientationModeFixedLandscape);
                case VideoAgoraProfile.Landscape480P:
                    return new VideoEncoderConfiguration(VideoEncoderConfiguration.VD840x480, VideoEncoderConfiguration.FRAME_RATE.FrameRateFps15,
                        VideoEncoderConfiguration.StandardBitrate, VideoEncoderConfiguration.ORIENTATION_MODE.OrientationModeFixedLandscape);
                case VideoAgoraProfile.Landscape360P:
                    return new VideoEncoderConfiguration(VideoEncoderConfiguration.VD480x360, VideoEncoderConfiguration.FRAME_RATE.FrameRateFps15, 
                        VideoEncoderConfiguration.StandardBitrate, VideoEncoderConfiguration.ORIENTATION_MODE.OrientationModeFixedLandscape);
                case VideoAgoraProfile.Landscape240P:
                    return new VideoEncoderConfiguration(VideoEncoderConfiguration.VD320x240, VideoEncoderConfiguration.FRAME_RATE.FrameRateFps15, 
                        VideoEncoderConfiguration.StandardBitrate, VideoEncoderConfiguration.ORIENTATION_MODE.OrientationModeFixedLandscape);
                case VideoAgoraProfile.Landscape120P:
                    return new VideoEncoderConfiguration(VideoEncoderConfiguration.VD160x120, VideoEncoderConfiguration.FRAME_RATE.FrameRateFps15, 
                        VideoEncoderConfiguration.StandardBitrate, VideoEncoderConfiguration.ORIENTATION_MODE.OrientationModeFixedLandscape);
        ...
        public virtual void OnFirstRemoteVideoDecoded(int uid, int width, int height, int elapsed)
        {
            ...
            if (!_containers.Any(a => a.StreamUID == id))
            {
                if (_containers.Any(a => a.StreamUID == AgoraService.UnknownRemoteStreamId)) {
                    AgoraVideoViewHolder<FrameLayout> viewHolder = _containers.First(a => a.StreamUID == AgoraService.UnknownRemoteStreamId);
        ...
        {
            if (!_knownStreams.Contains(holder.StreamUID) 
                && holder.StreamUID != AgoraService.UnknownRemoteStreamId)
            {
                return;
            }
            ...
            if (holder.StreamUID != AgoraService.UnknownRemoteStreamId) {
        ...
        internal void OnUserOffline(int uid, int reason)
        {
            ...
            List<AgoraVideoViewHolder<FrameLayout>> toClear = _containers.Where(a => a.StreamUID == id && a.IsStatic).ToList();
            List<AgoraVideoViewHolder<FrameLayout>> toRemove = _containers.Where(a => a.StreamUID == id && !a.IsStatic).ToList();
            ...
            toClear.ForEach(container =>
            {
                container.VideoView.IsOffline = true;
                container.VideoView.IsAudioMuted = false;
                container.VideoView.IsVideoMuted = false;
                container.VideoView.StreamUID = container.VideoView.StreamUID == AgoraService.UnknownLocalStreamId 
                                                ? AgoraService.UnknownLocalStreamId : AgoraService.UnknownRemoteStreamId;
            });
            OnDisconnected(id);
        }
        ...
        {
            uint id = (uint)uid;
            _containers.Where(a => a.StreamUID == id).ToList().ForEach(holder => {
